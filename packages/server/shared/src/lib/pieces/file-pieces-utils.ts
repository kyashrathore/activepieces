...
export const filePiecesUtils = (packages: string[], log: FastifyBaseLogger) => {
  ...
  async function getPackageNameFromFolderPath(
    folderPath: string
  ): Promise<string> {
  ...
  async function findPieceDirectoryByFolderName(
    pieceName: string
  ): Promise<string | null> {
  ...
  async function findAllPieces(): Promise<PieceMetadata[]> {
    const pieces = await loadPiecesFromFolder(
      resolve(cwd(), 'dist', 'packages', 'pieces')
    );
    return pieces;
  }

  async function loadPiecesFromFolder(
    folderPath: string
  ): Promise<PieceMetadata[]> {
    try {
      const paths = (await findAllPiecesFolder(folderPath)).filter((p) =>
        packages.some((packageName) => p.includes(packageName))
      );
      const pieces = await Promise.all(
        paths.map((p) => loadPieceFromFolder(p))
      );
      console.log({ pieces });
      ...
      log.warn({
        name: 'FilePieceMetadataService#loadPiecesFromFolder',
        message: err.message,
        stack: err.stack,
      });
      return [];
  ...
  async function loadPieceFromFolder(
    ...
    try {
      ...
      const module = importFresh<Record<string, unknown>>(indexPath);
